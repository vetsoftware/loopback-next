...
export abstract class ExtensionPoint<EXT extends object> {
   ...
   */
  @inject.config() protected config: ExtensionPointConfig = {};
   ...
   */
  getAllExtensionBindings(ctx: Context): Binding[] {
   ...
   */
  getExtensionBindingMap(ctx: Context): {[name: string]: Binding} {
    const extensionBindingMap: {[name: string]: Binding} = {};
   ...
   */
  getExtensionBinding(ctx: Context, extensionName: string): Binding {
    const bindings = this.getAllExtensionBindings(ctx);
